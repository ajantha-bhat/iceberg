...
public abstract class BaseTestIceberg {
  ...
  static String viewMetadataLocation(NessieCatalog catalog, TableIdentifier tableIdentifier) {
    View view = catalog.loadView(tableIdentifier);
    BaseView baseView = (BaseView) view;
    ViewOperations ops = baseView.operations();
    NessieViewOperations icebergOps = (NessieViewOperations) ops;
    return icebergOps.current().metadataFileLocation();
  }
