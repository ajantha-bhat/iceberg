...
public class TestPartitionStats extends TableTestBase {
  ...
  @Test
  public void testPartitionStatsOnPartitionColumnsWithSpace() throws Exception {
    // Schema passed to create tables
    final Schema schema =
        new Schema(
            required(3, "space id", Types.IntegerType.get()),
            required(4, "2!space data", Types.StringType.get()));

    // Partition spec used to create tables
    final PartitionSpec spec =
        PartitionSpec.builderFor(schema)
            .identity("space id", "space id")
            .identity("2!space data", "2!space data")
            .build();

    TestTables.TestTable tb =
        TestTables.create(temp.newFolder(), "partitioned", schema, spec, formatVersion);

    final DataFile fileA =
        DataFiles.builder(spec)
            .withPath("/path/to/data-a.parquet")
            .withFileSizeInBytes(10)
            .withPartitionPath("space id=0/2!space data='hello'")
            .withRecordCount(5)
            .build();

    tb.newAppend().appendFile(fileA).commit();

    validatePartitionStaticsFile(tb.currentSnapshot(), tb);

    PartitionData partitionData = new PartitionData(Partitioning.partitionType(tb));
    partitionData.set(0, 0);
    partitionData.set(1, "'hello'");
    Partition partition1 =
        buildPartition(partitionData, 5, 1, 10, tb.currentSnapshot(), tb.spec().specId());
    validatePartitions(tb, Collections.singletonList(partition1));

    final DataFile fileB =
        DataFiles.builder(spec)
            .withPath("/path/to/data-b.parquet")
            .withFileSizeInBytes(5)
            .withPartitionPath("space id=1/2!space data='world'")
            .withRecordCount(10)
            .build();
    tb.newAppend()
        .appendManifest(writeManifest(tb, "new-manifest0.avro", manifestEntry(ADDED, null, fileB)))
        .commit();
    validatePartitionStaticsFile(tb.currentSnapshot(), tb);
    partitionData = new PartitionData(Partitioning.partitionType(tb));
    partitionData.set(0, 1);
    partitionData.set(1, "'world'");
    Partition partition2 =
        buildPartition(partitionData, 10, 1, 5, tb.currentSnapshot(), tb.spec().specId());
    validatePartitions(tb, Lists.newArrayList(partition1, partition2));
  }
  ...
  private void validatePartitionStaticsFile(Snapshot snapshot, Table table) {
    ImmutablePartitionStatisticsFile partitionStatisticsFile =
        ImmutablePartitionStatisticsFile.builder()
