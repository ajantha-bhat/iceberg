...
public class PartitionStatsRecord implements Record, StructLike {
  ...
  public String toString() {
    ...
    sb.append("Record(");
    for (int i = 0; i < partitionStats.size(); i += 1) {
      if (i != 0) {
        sb.append(", ");
      }
      sb.append(partitionStats.get(i, Object.class));
    }
    sb.append(")");
    return sb.toString();
  }
  ...
  public static PartitionData recordToPartitionData(Record partitionRecord) {
    PartitionData partitionData = new PartitionData(partitionRecord.struct());
    for (int index = 0; index < partitionRecord.size(); index++) {
      partitionData.set(index, partitionRecord.get(index));
    }
  ...
  public static Record partitionDataToRecord(
          PartitionData partitionData, Types.StructType partitionType) {
    GenericRecord partitionRecord = GenericRecord.create(partitionType);
    if (partitionData != null) {
      for (int index = 0; index < partitionData.getSchema().getFields().size(); index++) {
        Object val = partitionData.get(index);
        if (val != null) {
          partitionRecord.set(
                  index,
