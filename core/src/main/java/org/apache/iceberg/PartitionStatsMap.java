...
/** Holds updated delta information - can be used for both deletes and inserts. */
class PartitionStatsMap {
  ...
  private void updatePartitionMap(ContentFile<?> file) {
    ...
    if (!specPartitionType.equals(pd.getPartitionType())
        && !hasSamePartitionFieldsWithNameSanitization(specPartitionType, pd.getPartitionType())) {
    ...
    // from SnapshotProducer#getUpdatedPartitionStatsLocation()
    partitionMap.get(partition).update(file, null);
  }

  /** Field names read from avro files are sanitized. */
  private static boolean hasSamePartitionFieldsWithNameSanitization(
      Types.StructType specPartitionType, Types.StructType filePartitionType) {
    List<Types.NestedField> specPartitionFieldsWithNameMangling =
        specPartitionType.fields().stream()
            .map(
                f ->
                    AvroSchemaUtil.validAvroName(f.name())
                        ? f
                        : Types.NestedField.of(
                            f.fieldId(),
                            f.isOptional(),
                            AvroSchemaUtil.sanitize(f.name()),
                            f.type(),
                            f.doc()))
            .collect(Collectors.toList());

    return specPartitionFieldsWithNameMangling.equals(filePartitionType.fields());
  }
}
