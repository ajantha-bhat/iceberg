...
public class PartitionStats implements StructLike {
   ...
   */
  public synchronized void liveEntry(ContentFile<?> file, Snapshot snapshot) {
    ...
    if (snapshot != null) {
      if (lastUpdatedAt == null || (lastUpdatedAt < snapshot.timestampMillis())) {
   ...
   */
  public synchronized void deletedEntry(Snapshot snapshot) {
